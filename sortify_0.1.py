...
def update_metadata(file_path):
    """
    Takes the file path as an input and searches it for a date in the 
    forms: YYYY-MM-DD HH mm ss, YYYY MM DD HH mm ss, YYYY-MM-DD HH:mm:ss 
    and YYYY-MM-DD (using 00:00:00 as default time values). 
    
    It takes this date string and puts it in datetime format then applies 
    the new datetime to the file metadata and prints a confirmation output. 
    If a date cannnot be found in the filename it will print an error message and move on. 
    ...
    try:
        ...
        if date_match:
            ...
            date_match = re.search(r'(\d{4}-\d{2}-\d{2})|(\d{4} \d{2} \d{2})', file_path)
            if date_match:
                creation_date_str = date_match.group(1)
                creation_date = datetime.datetime.strptime(creation_date_str, '%Y-%m-%d')

                # update creation date in file metadata using subprocess exiftool
                subprocess.run(['exiftool', '-FileModifyDate=' + creation_date.strftime('%Y:%m:%d 00:00:00'), file_path])
